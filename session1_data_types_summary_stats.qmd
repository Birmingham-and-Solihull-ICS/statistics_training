...
$$ sd = \sqrt{ \frac{\sum_{i=1}^n \left(x_i - \bar{x}^2 \right)}{n-1}}$$
or, in easier terms:
...
lines_dt <-
  data.frame(measure = c("mean", "median", "model", "mean", "median", "model"),
             value = c(mean(skew$l), median(skew$l), getmode(skew$l), mean(skew$r), median(skew$r), getmode(skew$r)),
...
s <- 
  s + 
  geom_vline(aes(xintercept = value, linetype = measure), col= "black", data = lines_dt) +
    guides(fill = FALSE) 
